...
def test_registered_routes():
    ...
    vision_paths = {r.path for r in vision_router.routes}
    ...
    assert "/api/check-take" in vision_paths
    assert "/api/compare-takes" in vision_paths
    assert "/api/upload-script" in vision_paths
    ...
    print(f"  OK: vision_router ({len(vision_paths)} routes)")
